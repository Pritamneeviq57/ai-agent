...
def process_meetings():
    ...
    try:
        ...
        # Fetch meetings from Graph API (last 7 days to now, using user ID)
        logger.info("📅 Fetching Teams meetings from Graph API (last 7 days to now)...")
        if USE_DELEGATED_AUTH:
            ...
            if TARGET_USER_ID:
                ...
                all_meetings = fetcher.list_all_meetings_with_transcripts(days_back=7, limit=50, user_id=TARGET_USER_ID)
            else:
                all_meetings = fetcher.list_all_meetings_with_transcripts(days_back=7, limit=50)
        else:
            ...
            logger.info(f"🎯 Using specific user ID: {TARGET_USER_ID}")
            all_meetings = fetcher.list_all_meetings_with_transcripts(user_id=TARGET_USER_ID, days_back=7, limit=50)
